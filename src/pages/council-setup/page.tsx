import { useState } from "react";
import { AnimatePresence, motion } from "framer-motion";
import { AgentPods } from "@/components/council-setup/agent-pods";
import { SetupSlider } from "@/components/council-setup/setup-slider";
// import { VoiceGlow } from "@/components/council-setup/voice-glow";
import { CouncilOverview } from "@/components/council-overview/Council-overview";
import { Mic, SlidersHorizontal } from "lucide-react";



type Phase = "voice" | "manual";

export default function CouncilSetup() {
  const [phase, setPhase] = useState<Phase>("voice");

  return (
    <main className="2xl:container w-11/12 mx-auto">
      {/* Header Badge */}
      <div className="glass w-56 h-8.5 rounded-full flex items-center justify-center gap-2 text-[#7F0DF2] font-semibold uppercase mt-10">
        <svg
          width="13"
          height="13"
          viewBox="0 0 13 13"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M10.5 4.6667L9.77083 3.06253L8.16667 2.33336L9.77083 1.6042L10.5 3.05176e-05L11.2292 1.6042L12.8333 2.33336L11.2292 3.06253L10.5 4.6667V4.6667M10.5 12.8334L9.77083 11.2292L8.16667 10.5L9.77083 9.77086L10.5 8.1667L11.2292 9.77086L12.8333 10.5L11.2292 11.2292L10.5 12.8334V12.8334M4.66667 11.0834L3.20833 7.87503L0 6.4167L3.20833 4.95836L4.66667 1.75003L6.125 4.95836L9.33333 6.4167L6.125 7.87503L4.66667 11.0834V11.0834M4.66667 8.2542L5.25 7.00003L6.50417 6.4167L5.25 5.83336L4.66667 4.5792L4.08333 5.83336L2.82917 6.4167L4.08333 7.00003L4.66667 8.2542V8.2542M4.66667 6.4167V6.4167V6.4167V6.4167V6.4167V6.4167V6.4167V6.4167V6.4167V6.4167"
            fill="#7F0DF2"
          />
        </svg>
        <p className="text-xs">Configuration Module</p>
      </div>

      {/* Title Section */}
      <div className="my-7 space-y-2">
        <h1 className="text-2xl md:text-3xl font-bold text-white">
          Configure Your <span className="text-[#7F0DF2]">Council.</span>
        </h1>
        <p className="text-gray-400 text-[10px] max-w-md">
          {phase === "voice"
            ? "Speak to the Manager Agent to architect your team. Describe your vision and let AI suggest the perfect lineup."
            : "Orchestrate your high-performance team of specialized AI agents. Adjust the hierarchy and define the cognitive parameters for maximum strategic output."}
        </p>
      </div>

      {/* Phase Toggle */}
      <div className="flex gap-2 mb-3">
        <motion.button
          onClick={() => setPhase("voice")}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer ${
            phase === "voice"
              ? "bg-[#7F0DF2] text-white shadow-lg shadow-[#7F0DF2]/25"
              : "glass text-white/50 hover:text-white/80"
          }`}
          whileTap={{ scale: 0.97 }}
        >
          <Mic className="w-3.5 h-3.5" />
          Voice Setup
        </motion.button>
        <motion.button
          onClick={() => setPhase("manual")}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer ${
            phase === "manual"
              ? "bg-[#7F0DF2] text-white shadow-lg shadow-[#7F0DF2]/25"
              : "glass text-white/50 hover:text-white/80"
          }`}
          whileTap={{ scale: 0.97 }}
        >
          <SlidersHorizontal className="w-3.5 h-3.5" />
          Manual Setup
        </motion.button>
      </div>

      {/* Phase Content */}
      <AnimatePresence mode="wait">
        {phase === "voice" ? (
          // <motion.div
          //   key="voice"
          //   initial={{ opacity: 0, y: 20 }}
          //   animate={{ opacity: 1, y: 0 }}
          //   exit={{ opacity: 0, y: -20 }}
          //   transition={{ duration: 0.4 }}
          // >
          //   <section className="glass w-full p-6 rounded-2xl min-h-[500px] flex items-center justify-center">
          //     <VoiceGlow onComplete={() => setPhase("manual")} />
          //   </section>
          // </motion.div>
          <div>
            voice
          </div>
        ) : (
          <motion.div
            key="manual"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            transition={{ duration: 0.4 }}
          >
            {/* Council Size Slider */}
            <section className="glass w-full p-5 rounded-2xl">
              <div className="lg:flex items-center justify-between gap-10">
                <div className="flex-1 space-y-3">
                  <div className="flex items-center gap-2">
                    <svg
                      width="24"
                      height="12"
                      viewBox="0 0 24 12"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M0 12V10.425C0 9.70833 0.366667 9.125 1.1 8.675C1.83333 8.225 2.8 8 4 8C4.21667 8 4.425 8.00417 4.625 8.0125C4.825 8.02083 5.01667 8.04167 5.2 8.075C4.96667 8.425 4.79167 8.79167 4.675 9.175C4.55833 9.55833 4.5 9.95833 4.5 10.375V12H0ZM6 12V10.375C6 9.84167 6.14583 9.35417 6.4375 8.9125C6.72917 8.47083 7.14167 8.08333 7.675 7.75C8.20833 7.41667 8.84583 7.16667 9.5875 7C10.3292 6.83333 11.1333 6.75 12 6.75C12.8833 6.75 13.6958 6.83333 14.4375 7C15.1792 7.16667 15.8167 7.41667 16.35 7.75C16.8833 8.08333 17.2917 8.47083 17.575 8.9125C17.8583 9.35417 18 9.84167 18 10.375V12H6ZM19.5 12V10.375C19.5 9.94167 19.4458 9.53333 19.3375 9.15C19.2292 8.76667 19.0667 8.40833 18.85 8.075C19.0333 8.04167 19.2208 8.02083 19.4125 8.0125C19.6042 8.00417 19.8 8 20 8C21.2 8 22.1667 8.22083 22.9 8.6625C23.6333 9.10417 24 9.69167 24 10.425V12H19.5ZM8.125 10H15.9C15.7333 9.66667 15.2708 9.375 14.5125 9.125C13.7542 8.875 12.9167 8.75 12 8.75C11.0833 8.75 10.2458 8.875 9.4875 9.125C8.72917 9.375 8.275 9.66667 8.125 10ZM4 7C3.45 7 2.97917 6.80417 2.5875 6.4125C2.19583 6.02083 2 5.55 2 5C2 4.43333 2.19583 3.95833 2.5875 3.575C2.97917 3.19167 3.45 3 4 3C4.56667 3 5.04167 3.19167 5.425 3.575C5.80833 3.95833 6 4.43333 6 5C6 5.55 5.80833 6.02083 5.425 6.4125C5.04167 6.80417 4.56667 7 4 7ZM20 7C19.45 7 18.9792 6.80417 18.5875 6.4125C18.1958 6.02083 18 5.55 18 5C18 4.43333 18.1958 3.95833 18.5875 3.575C18.9792 3.19167 19.45 3 20 3C20.5667 3 21.0417 3.19167 21.425 3.575C21.8083 3.95833 22 4.43333 22 5C22 5.55 21.8083 6.02083 21.425 6.4125C21.0417 6.80417 20.5667 7 20 7ZM12 6C11.1667 6 10.4583 5.70833 9.875 5.125C9.29167 4.54167 9 3.83333 9 3C9 2.15 9.29167 1.4375 9.875 0.8625C10.4583 0.2875 11.1667 0 12 0C12.85 0 13.5625 0.2875 14.1375 0.8625C14.7125 1.4375 15 2.15 15 3C15 3.83333 14.7125 4.54167 14.1375 5.125C13.5625 5.70833 12.85 6 12 6ZM12 4C12.2833 4 12.5208 3.90417 12.7125 3.7125C12.9042 3.52083 13 3.28333 13 3C13 2.71667 12.9042 2.47917 12.7125 2.2875C12.5208 2.09583 12.2833 2 12 2C11.7167 2 11.4792 2.09583 11.2875 2.2875C11.0958 2.47917 11 2.71667 11 3C11 3.28333 11.0958 3.52083 11.2875 3.7125C11.4792 3.90417 11.7167 4 12 4Z"
                        fill="#7F0DF2"
                      />
                    </svg>
                    <p className="font-semibold text-white">
                      Global Council Size
                    </p>
                  </div>
                  <p className="text-[10px] text-gray-400">
                    Define the total number of specialized agents in your
                    assembly.
                  </p>
                </div>
                <div className="flex-1 lg:max-w-md">
                  <SetupSlider />
                </div>
              </div>
            </section>

            {/* Agent Pods */}
            <section className="w-full">
              <AgentPods />
            </section>
          </motion.div>
        )}
      </AnimatePresence>

      {/* Council Overview Modal */}
      <section>
        <CouncilOverview />
      </section>
    </main>
  );
}
